...
| LEG-03 | Corpus manifest for raw replays, results, and replay list. | Local raw corpus has 23,473 JSON files; `replaysList.json` has 23,456 replay rows; existing results tree has 88,485 files and a 51,717,050-byte `stats.zip`. [VERIFIED: local `find`, `node`, and `wc` commands] |
...
| `~/sg_stats/lists/replaysList.json` has 23,456 replay rows and top-level keys `replaysListPreparedAt`, `parsedReplays`, `replays`, and `problematicReplays`. [VERIFIED: local Node JSON inspection] | Manifest must explicitly reconcile 23,473 raw files versus 23,456 listed replay rows. [VERIFIED: local raw/list comparison] |
